...
public class PromptUtil {
	...
	public String TPQ(String text) {
		...
		return String.join("", Arrays.asList(
			...
			"**공통 요구 사항**\n",
			"- 간결한 질문: 모든 질문은 간결하면서도 학습적 가치를 극대화할 수 있도록 작성하며, 사용자가 이해하기 쉽고 기억에 잘 남도록 구성한다. 광범위한 Answer가 나올 질문보다는, 특정한 주제 하나로 답변이 나올 수 잇도록 한다.\n",
			"- TPQ 형식의 질문 구성: 각 질문은 본문에서 중요하거나 핵심적인 개념을 다루어야 하며, 사용자가 학습 내용을 쉽게 기억할 수 있도록 작성한다. 특정한 키워드 1~2개를 Answer로 답할 수 있는 질문을 낸다.\n",
			"- 사용자가 TTS로 들을 때 이해하기 쉬운 리듬과 구조를 제공한다.\n", // 듣기 쉬운 대본
			"- 주제와 포인트 명확화: 각 질문(Q) 앞에는 P로 시작하는 포인트(P)를, 각 대화 주제 앞에는 T로 시작하는 타이틀(T)을 사용하여, 주제를 명확히 설정한다.\n",
			// 주제와 포인트 명확화
			"- 단일 주제 그룹화: 비슷한 주제의 질문은 반드시 하나의 대제목(T)에 묶어야 한다. 즉, 질문은 반드시 하나의 T에 소속되어야 한다.\n",
			"- 영어 알파벳을 제외한 일본어, 중국어와 같은 외국어 및 한자 등을 포함하여 반환하지 말 것.\n",
			"- T와 P와 Q를 기준으로, 하나의 개행문자(\\n)로 나누어 응답할 것.\n",
			"- \"다른 표현 방식들\"과 같은 추가적인 표현 방식이나 \"이 TPQ의 작성 이유는 다음과 같습니다:\"와 같은 프롬프트 설명 멘트를 포함하여 반환하지 말 것.\n",
	...
	public String getKeywordTPQ(String keyword) {
		...
		return String.join("", Arrays.asList(
			"당신은 자연어 처리의 전문가로서, 주어진 키워드를 바탕으로 학습에 효과적인 TPQ 스크립트를 작성해야 한다\n",
			"반환 데이터는 T(Title), P(Point), Q(Question)로 구성된 형식이어야 하며, 사용자가 해당 키워드에 대해 학습적인 효과를 극대화할 수 있도록 구성해야 한다.\n",
			"사용자가 학습 내용을 쉽게 이해하고 기억할 수 있도록, 질문과 포인트가 명확히 드러나도록 작성하라.\n",

			"TPQ 형식의 텍스트에 포함될 것:\n",
			"- 대제목(T): 입력된 키워드를 중심으로 주요 주제를 설정해야 한다. 간결하면서도 포괄적으로 주제를 요약해야 한다.\n",
			"- 포인트(P): 각 질문(Q)이 다루는 세부 주제 및 키워드를 명확히 설정한다. 핵심 개념이 강조되도록 간결하게 작성할 것.\n",
			"- 질문(Q): 시험이나 면접 등에서 자주 다뤄지는 키워드의 핵심 개념이나 외워야 할 부분에 대한 질문을 명확하고 간결하게 작성한다. ",
			"학습 및 암기에 용이하도록 작성해야 한다.\n",
	...
	private String getSimpleAnswerRequirements() {
		...
		return String.join("", Arrays.asList(
			...
			"- 'DML'에 대해서 질문했을 때, 'DML(Data Manipulation Language)!'로 답변하는 것처럼, 질문의 핵심 키워드 자체만을 답변하는 것은 지양한다.\n", // 핵심 키워드만
