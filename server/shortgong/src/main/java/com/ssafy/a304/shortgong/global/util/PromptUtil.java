...
public class PromptUtil {
	...
	public String complete(String text) {
		...
		return String.join("", Arrays.asList(
			...
			"QA 형식의 텍스트에는 다음과 같은 요소들이 포함되어야 합니다:\n",
			"**Simple 버전 요구 사항**\n",
	...
	public String simple(String text) {
		...
		return String.join("", Arrays.asList(
				...
				"- 팟캐스트 형식을 가진 QA 스크립트라는 점을 명심하세요. QA 형식이라는 것에 주안점을 두고 작성해 주세요.\n",
				"- 주제와 포인트 명확화: 각 질문(Q) 앞에는 P로 시작하는 포인트(P)를, 각 대화 주제(TITLE) 앞에는 T로 시작하는 타이틀(T)을 사용하여, ",
				"주제를 명확히 설정합니다. 이는 TTS로 제공될 때 사용자가 각 주제를 쉽게 이해하고 기억하도록 돕습니다.\n",
				"- 비슷한 주제의 질문은 반드시 하나의 T로 묶어주어야 합니다. 즉, 질문은 반드시 하나의 T에 소속되어 있어야 합니다.\n",
				...
				"사용자가 TTS로 들었을 때도 충분히 암기적 효과를 얻을 수 있도록 구성해 주세요.\n",
				"전체적인 QA는 학습적 가치를 극대화할 수 있도록 전체적인 흐름이 잘 정리되어야 합니다. ",
				"이를 통해 사용자는 텍스트를 TTS로 듣는 것만으로도 깊이 있는 지식을 효과적으로 습득할 수 있습니다.\n\n",
	...
	public String detail(String text) {
		...
		return String.join("", Arrays.asList(
			...
			"- \"다른 표현 방식들\", \"작성 이유는 다음과 같습니다:\"과 같은 추가적인 표현 방식을 제안하지 말 것.\n",
			"- 영어 알파벳을 제외한 일본어, 중국어와 같은 외국어 및 한자 등을 포함하여 반환하지 말 것.\n",
			"- 각 진행자의 스크립트를 기준으로 개행문자(\\\"\\n\\\")로 나누어 응답할 것.\n",
			"- 질문자의 스크립트 앞에 \"Q: \"를, 답변자의 스크립트 앞에 \"A: \"를 붙여 반환할 것.\n\n",
